...
def callback_scan(msg):
    ...
    points = []
    #Umbral de distancia de la lectura, mas alla de 50 cm no se toma en cuenta
    th_dist = 0.50
    for i in range(len(msg.ranges)):
        #Si la lectura esta mas lejos que 50cm entonces no lo toma en cuenta
        if not (math.isnan(msg.ranges[i]) or msg.ranges[i] >= msg.range_max) and (msg.ranges[i] < th_dist):
            r,theta = msg.ranges[i], i*msg.angle_increment + msg.angle_min + a
            points.append([r*math.cos(theta) + x, r*math.sin(theta) + y])
    points = numpy.asarray(points)
    #
    #Aqui estan los resultados de las lineas
    #
    lines = split_and_merge(points, distance_threshold, min_points_counting, rho_tolerance, theta_tolerance)
    vel = Twist()
    vel.angular.z = lines[0][0]
    print("Va a publicar la vel")
    if giro_flag:
        pub_vel.publish(vel)
        giro_flag = False
...
def main():
    global pub_line_markers, listener, distance_threshold, min_points_counting, rho_tolerance, theta_tolerance, giro_flag
    giro_flag = True
    ...
    #
    distance_threshold  = rospy.get_param("~dist", 0.05)     #Distance threshold to consider a point as part of a candidate line. 
    min_points_counting = rospy.get_param("~points", 30)     #Minimum number of points a line should contain.
